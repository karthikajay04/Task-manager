...
<main class="app-shell">
  ...
  <section class="workspace">
    ...
    <section class="list-head"><h2>{{ filterLabel() }}</h2><span>{{ visibleTasks().length }} {{ visibleTasks().length === 1 ? 'item' : 'items' }}</span></section>
...
export class DashboardComponent implements OnInit {
  tasks = signal<Task[]>([]); filter = signal<Filter>('all'); showComposer = signal(false); editing = signal<Task | null>(null); saving = signal(false); today = new Date(); priorities: TaskDraft['priority'][] = ['low', 'medium', 'high'];
  draft: TaskDraft = this.newDraft();
  visibleTasks = computed(() => this.tasks().filter(t => this.filter() === 'all' || this.filter() === 'done' ? this.filter() === 'all' || t.completed : !t.completed));
