...
<main class="app-shell">
  ...
  <section class="workspace">
    ...
    <section class="list-head"><h2>{{ filterLabel() }}</h2><label class="search"><span>⌕</span><input [(ngModel)]="searchTerm" (ngModelChange)="search.set($event)" placeholder="Search your notes"></label><span>{{ visibleTasks().length }} {{ visibleTasks().length === 1 ? 'item' : 'items' }}</span></section>
    <section class="task-list">
      @for (task of visibleTasks(); track task._id) { <article class="task-card" [class.is-done]="task.completed"><button class="check" [class.checked]="task.completed" (click)="toggleTask(task)">{{ task.completed ? '✓' : '' }}</button><div class="task-copy"><div class="task-title-row"><h3>{{ task.title }}</h3><span class="priority" [class]="task.priority">{{ task.priority }}</span></div>@if (task.notes) { <p>{{ task.notes }}</p> }<div class="meta"><span class="category">{{ task.category }}</span>@if (task.dueDate) { <span>◷ {{ task.dueDate | date:'MMM d' }}</span> }</div></div><button class="more" (click)="edit(task)" aria-label="Edit task">•••</button></article> }
...
export class DashboardComponent implements OnInit {
  tasks = signal<Task[]>([]); filter = signal<Filter>('all'); search = signal(''); searchTerm = ''; showComposer = signal(false); editing = signal<Task | null>(null); saving = signal(false); today = new Date(); priorities: TaskDraft['priority'][] = ['low', 'medium', 'high'];
