...
export class AuthService {
  ...
  login(payload: { email: string; password: string }) { return this.http.post<{token: string; user: User}>(`${API_URL}/api/auth/login`, payload).pipe(tap(data => this.persist(data))); }
  register(payload: { name: string; email: string; password: string }) { return this.http.post<{token: string; user: User}>(`${API_URL}/api/auth/register`, payload).pipe(tap(data => this.persist(data))); }
